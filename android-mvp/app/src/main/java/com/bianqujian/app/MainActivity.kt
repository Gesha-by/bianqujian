...
class MainActivity : Activity() {
    ...
    private fun render() {
        ...
        val simulate = Button(this).apply { text = "模拟到件通知（测试）"; textSize = 10f; setTextColor(Color.rgb(104,119,146)); background = rounded(Color.rgb(242,245,250), 10f); setOnClickListener { simulateArrival() } }
    ...
    private fun refresh() { list.removeAllViews(); val found = parcels.count { it.found }; summary.text = if (parcels.isEmpty()) "找件清单 · 0 件" else "找件清单 · 已找到 $found / ${parcels.size}"; parcels.forEach { parcel -> val row = CheckBox(this).apply { text = "${parcel.code} · ${parcel.name}"; textSize = 14f; isChecked = parcel.found; setTextColor(if (parcel.found) Color.rgb(17,132,91) else Color.rgb(23,35,61)); setPadding(8, 13, 8, 13); setBackgroundColor(if (parcel.found) Color.rgb(241,255,248) else Color.WHITE); setOnCheckedChangeListener { _, checked -> parcel.found = checked; save(); refresh() } }; list.addView(row, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT).apply { bottomMargin = 8 }) } }
