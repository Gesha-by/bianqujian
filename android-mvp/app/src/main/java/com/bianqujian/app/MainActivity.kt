...
class MainActivity : Activity() {
    ...
    private fun render() {
        ...
        val hero = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(18, 17, 18, 17); background = rounded(Color.rgb(66,99,235), 22f) }
        ...
        val import = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(14, 11, 8, 11); background = rounded(Color.WHITE, 16f) }
        ...
        import.addView(Button(this).apply { text = "识别图片"; textSize = 11f; setTextColor(Color.rgb(66,99,235)); background = rounded(Color.rgb(237,241,255), 10f); setOnClickListener { chooseText() } })
        val simulate = Button(this).apply { text = "模拟到件数据（测试）"; textSize = 10f; setTextColor(Color.rgb(104,119,146)); background = rounded(Color.rgb(242,245,250), 10f); setOnClickListener { simulateArrival() }; visibility = if ((applicationInfo.flags and ApplicationInfo.FLAG_DEBUGGABLE) != 0) View.VISIBLE else View.GONE }
        ...
        val completedTitle = TextView(this).apply { text = "已完成"; textSize = 14f; setTextColor(Color.rgb(23,35,61)); setTypeface(null, 1); setPadding(2, 15, 2, 7) }
        ...
        val handoff = Button(this).apply { text = "打开拼多多扫码出库"; textSize = 14f; setTextColor(Color.WHITE); background = rounded(Color.rgb(23,35,61), 18f); setPadding(16, 16, 16, 16); setOnClickListener { Toast.makeText(this@MainActivity, "请在拼多多完成扫码出库", Toast.LENGTH_SHORT).show() } }
        root.addView(title); root.addView(hero); root.addView(import, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT).apply { topMargin = 14 }); root.addView(simulate, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT).apply { topMargin = 7 }); root.addView(summary); root.addView(list); root.addView(completedTitle); root.addView(completedList, LinearLayout.LayoutParams(-1, 0, 1f)); root.addView(handoffNote); root.addView(handoff, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT).apply { topMargin = 6 }); setContentView(root); refresh()
    }
    ...
    private fun addParcelRow(container: LinearLayout, parcel: Parcel, checked: Boolean) { val row = CheckBox(this).apply { text = "${parcel.code} · ${parcel.name}"; textSize = 14f; isChecked = checked; setTextColor(if (checked) Color.rgb(17,132,91) else Color.rgb(23,35,61)); setPadding(8, 13, 8, 13); setBackgroundColor(if (checked) Color.rgb(241,255,248) else Color.WHITE); setOnCheckedChangeListener { _, value -> if (parcel.found != value) { parcel.found = value; save(); refresh() } } }; container.addView(row, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT).apply { bottomMargin = 8 }) }
