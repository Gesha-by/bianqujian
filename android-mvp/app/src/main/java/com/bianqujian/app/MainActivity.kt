...
class MainActivity : Activity() {
    ...
    private fun importImage(uri: Uri) { val image = InputImage.fromFilePath(this, uri); val recognizer = TextRecognition.getClient(ChineseTextRecognizerOptions.Builder().build()); recognizer.process(image).addOnSuccessListener { result -> val rawText = result.text; val text = rawText.uppercase().replace('－', '-').replace('—', '-').replace('–', '-'); val found = codePattern.matcher(text); val location = extractOcrLocation(rawText); val carrier = extractOcrCarrier(rawText); val tracking = extractOcrTracking(rawText); val updatedAt = SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.CHINA).format(Date()); val imagePath = saveImportedImage(uri); var added = 0; while (found.find()) { val code = found.group().replace(Regex("\\s+"), "").replace(Regex("-+"), "-"); if (parcels.none { it.code == code }) { parcels.add(Parcel(code, extractOcrName(rawText), false, ParcelStatus.READY, "截图识别", location, "普通快递", carrier, tracking, updatedAt, imagePath)); added++ } }; save(); refresh(); Toast.makeText(this, if (added == 0) "未识别到取件码，请确认截图包含类似 B8-5-153 或 A-302-8 的编码" else "识别完成，已导入 $added 个取件码，商品图和物流信息已保存", Toast.LENGTH_LONG).show() }.addOnFailureListener { Toast.makeText(this, "图片识别失败，请重试", Toast.LENGTH_LONG).show() } }
